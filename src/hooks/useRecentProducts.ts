  ...
  const { data: recentProducts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('products')
        .select('id, sku, name, tipo_id, grupo_id, subgrupo_id, family_id, class_id, unit_price, width, length, thickness, aliquota_ipi, fator_kg, unit_measure, ficha_tecnica, ncm:ncm_codes(aliquota_ipi_oficial)')
        .in('id', recentIds);
