    ...
    BEGIN
      ...
      IF v_existing_id IS NOT NULL THEN
        UPDATE products SET
          ...
          erp_empresa         = COALESCE((NULLIF(rec.raw_data->>'empresa', ''))::int, erp_empresa),
          tipo_item           = COALESCE(NULLIF(rec.raw_data->>'desc_tipo_item', ''), tipo_item),
          tipo_ficha          = COALESCE((NULLIF(rec.raw_data->>'codigo_tipo_ficha', ''))::int, tipo_ficha),
          ...
          erp_versao_roteiro  = COALESCE((NULLIF(rec.raw_data->>'codigo_roteiro', ''))::int, erp_versao_roteiro),
          ...
          unit_price          = COALESCE((NULLIF(rec.raw_data->>'preco_venda', ''))::numeric, unit_price),
          price_cash          = COALESCE((NULLIF(rec.raw_data->>'preco_ultima_venda', ''))::numeric, price_cash),
          weight              = COALESCE((NULLIF(rec.raw_data->>'peso_liquido', ''))::numeric, weight),
