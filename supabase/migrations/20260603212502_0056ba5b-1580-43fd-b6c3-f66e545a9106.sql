-- 1) Função batch para can_manage_portfolio
CREATE OR REPLACE FUNCTION public.can_manage_portfolio_batch(
  p_user_id uuid,
  p_owner_ids uuid[],
  p_entity_types text[]
)
RETURNS TABLE(owner_id uuid, entity_type text, allowed boolean)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT o AS owner_id,
         e AS entity_type,
         public.can_manage_portfolio(p_user_id, o, e) AS allowed
  FROM unnest(p_owner_ids) AS o
  CROSS JOIN unnest(p_entity_types) AS e
$$;

GRANT EXECUTE ON FUNCTION public.can_manage_portfolio_batch(uuid, uuid[], text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.can_manage_portfolio_batch(uuid, uuid[], text[]) TO service_role;

-- 2) Reescreve search_customers_paginated incluindo erp_code no retorno
