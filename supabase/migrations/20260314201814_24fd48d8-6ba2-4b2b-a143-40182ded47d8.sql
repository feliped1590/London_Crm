...
-- Drop both overloads and recreate with SET LOCAL row_security = off
...
-- Recreate single unified function with p_allowed_sales_rep_ids
...
BEGIN
  -- Disable RLS for this function's session so aggregate subqueries
  -- on deals, activities, tasks, orders etc. are not filtered by caller's permissions
  ...
  ORDER BY
    CASE WHEN p_sort_dir = 'asc' THEN
      CASE p_sort_field
        WHEN 'name' THEN coalesce(c.fantasia, c.name)
        WHEN 'contact' THEN coalesce(c.contact_name, '')
        WHEN 'phone' THEN coalesce(c.phone, '')
        WHEN 'owner' THEN coalesce(sr.name, '')
