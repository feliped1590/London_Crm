...
BEGIN
  ...
  -- Total + ids da página em uma única passagem na tabela companies
  CREATE TEMP TABLE IF NOT EXISTS _scp_filtered (id uuid) ON COMMIT DROP;
  TRUNCATE _scp_filtered;
  INSERT INTO _scp_filtered (id)
  SELECT c.id
  FROM companies c
  WHERE
  ...
  SELECT COUNT(*)::bigint INTO v_total FROM _scp_filtered;

  RETURN QUERY
  WITH page AS (
    SELECT c.*, sr.name AS sr_name
    FROM companies c
    JOIN _scp_filtered f ON f.id = c.id
  ...
  ORDER BY
    ...
    CASE WHEN v_sort_field = 'owner'        AND v_sort_dir = 'asc'  THEN lower(coalesce(p.sr_name,'')) END ASC  NULLS LAST,
    CASE WHEN v_sort_field = 'owner'        AND v_sort_dir = 'desc' THEN lower(coalesce(p.sr_name,'')) END DESC NULLS LAST,
