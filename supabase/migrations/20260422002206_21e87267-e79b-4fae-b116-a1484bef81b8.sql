-- create_app_session: enriquecer log com flow='create'
...
-- force_replace_session: enriquecer log com flow='replace' e session_id substituído
...
BEGIN
  ...
  IF NOT public.is_within_access_window(p_user_id) THEN
    ...
    SELECT id INTO v_replaced_session_id
    ...
    INSERT INTO public.access_violation_log (
      ...
      jsonb_build_object(
        ...
        'flow', 'replace',
        'replaced_session_id', v_replaced_session_id,
        'device_info', p_device_info,
        'attempted_at', now()
      )
    );
