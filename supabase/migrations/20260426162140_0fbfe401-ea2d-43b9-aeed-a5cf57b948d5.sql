...
BEGIN
  ...
  manager_filtered_users AS (
    SELECT mu.user_id, mu.tenant_id
    FROM public.manager_users mu
    JOIN user_tenants ut ON ut.tenant_id = mu.tenant_id
  ...
  raw_interactions AS (
    ...
    WHERE a.created_at >= p_start_date AND a.created_at < p_end_date
      ...
      AND (p_manager_user_id IS NULL OR EXISTS (
        SELECT 1 FROM manager_filtered_users mu
        WHERE mu.user_id = a.created_by AND mu.tenant_id = a.tenant_id
      ))
    UNION ALL
    ...
    WHERE t.created_at >= p_start_date AND t.created_at < p_end_date
      ...
      AND (p_manager_user_id IS NULL OR EXISTS (
        SELECT 1 FROM manager_filtered_users mu
        WHERE mu.user_id = t.created_by AND mu.tenant_id = t.tenant_id
      ))
    UNION ALL
    ...
    WHERE t.completed_at >= p_start_date AND t.completed_at < p_end_date AND t.completed_at IS NOT NULL
      ...
      AND (p_manager_user_id IS NULL OR EXISTS (
        SELECT 1 FROM manager_filtered_users mu
        WHERE mu.user_id = t.assigned_to AND mu.tenant_id = t.tenant_id
      ))
    UNION ALL
    ...
    WHERE dsh.changed_at >= p_start_date AND dsh.changed_at < p_end_date
      ...
      AND (p_manager_user_id IS NULL OR EXISTS (
        SELECT 1 FROM manager_filtered_users mu
        WHERE mu.user_id = dsh.changed_by AND mu.tenant_id = d.tenant_id
      ))
    UNION ALL
    ...
    WHERE pr.created_at >= p_start_date AND pr.created_at < p_end_date
      ...
      AND (p_manager_user_id IS NULL OR EXISTS (
        SELECT 1 FROM manager_filtered_users mu
        WHERE mu.user_id = pr.created_by AND mu.tenant_id = pr.tenant_id
      ))
    UNION ALL
    ...
    WHERE o.created_at >= p_start_date AND o.created_at < p_end_date
      ...
      AND (p_manager_user_id IS NULL OR EXISTS (
        SELECT 1 FROM manager_filtered_users mu
        WHERE mu.user_id = o.created_by AND mu.tenant_id = o.tenant_id
      ))
    UNION ALL
    ...
    WHERE en.created_at >= p_start_date AND en.created_at < p_end_date
      ...
      AND (p_manager_user_id IS NULL OR EXISTS (
        SELECT 1 FROM manager_filtered_users mu
        WHERE mu.user_id = en.created_by AND mu.tenant_id = d.tenant_id
      ))
    UNION ALL
    SELECT sub.changed_by, sub.tenant_id, 'deal_updates' FROM (
      ...
      WHERE dal.changed_at >= p_start_date AND dal.changed_at < p_end_date
        ...
        AND (p_manager_user_id IS NULL OR EXISTS (
          SELECT 1 FROM manager_filtered_users mu
          WHERE mu.user_id = dal.changed_by AND mu.tenant_id = d.tenant_id
        ))
    ) sub
    UNION ALL
    ...
    WHERE el.sent_at >= p_start_date AND el.sent_at < p_end_date
      ...
      AND (p_manager_user_id IS NULL OR EXISTS (
        SELECT 1 FROM manager_filtered_users mu
        WHERE mu.user_id = el.sent_by AND mu.tenant_id = el.tenant_id
      ))
  ),
  counts AS (
    SELECT ri.uid,
