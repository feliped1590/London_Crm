...
CREATE INDEX IF NOT EXISTS idx_companies_active_last_interaction
  ON public.companies (active, last_interaction_at DESC NULLS LAST);

-- 3. Reescrita da função search_customers_paginated (mesma assinatura)
...
-- 4. RPC agregada para Top Produtos (dashboard)
CREATE OR REPLACE FUNCTION public.dashboard_top_products(p_limit integer DEFAULT 5)
RETURNS TABLE(name text, total_quantity numeric)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path TO 'public'
AS $$
  ...
  GROUP BY COALESCE(p.name, oi.description)
  ORDER BY total_quantity DESC
  LIMIT GREATEST(p_limit, 1)
$$;

GRANT EXECUTE ON FUNCTION public.dashboard_top_products(integer) TO authenticated;
