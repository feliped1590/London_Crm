-- Refinamentos finais para company_products: índice extra e RPC com inteligência de compra + checagem explícita de tenant do usuário.

CREATE INDEX IF NOT EXISTS idx_cp_active_lookup
ON public.company_products (company_id, product_id)
WHERE archived_at IS NULL;

DROP FUNCTION IF EXISTS public.get_available_company_products(uuid, text, integer);

CREATE FUNCTION public.get_available_company_products(
  p_company_id uuid,
