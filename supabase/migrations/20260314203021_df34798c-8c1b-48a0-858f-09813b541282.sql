...
BEGIN
  ...
  RETURN QUERY
  WITH
  -- Aggregation CTEs: each table aggregated ONCE
  activities_agg AS (
  ...
  emails_agg AS (
    ...
    FROM email_logs el
    JOIN contacts ct ON el.contact_id = ct.id
  ...
  contacts_agg AS (
    SELECT
      ct.company_id,
      COUNT(*) AS total_count,
      MIN(ct.created_at) AS first_contact_created -- to get the "first" contact
    FROM contacts ct
    GROUP BY ct.company_id
  ),
  primary_contacts AS (
  ...
  -- Main base with JOINs instead of subqueries
  base AS (
    SELECT
      c.id AS c_id,
      c.name AS c_name,
      ...
      oa.last_order_at AS c_last_order_at,
