...
-- Update RLS: SELECT is open to all authenticated, UPDATE/DELETE restricted to owner/admin/sales_rep
-- First drop existing UPDATE and DELETE policies
...
-- Recreate UPDATE policy: admin OR owner OR created_by OR sales_rep linked to user OR delegation
...
FOR UPDATE USING (
  ...
  OR sales_rep_id IN (
    SELECT usr.sales_rep_id FROM public.user_sales_reps usr WHERE usr.user_id = auth.uid()
  )
);

-- Recreate DELETE policy: same logic
...
-- Update search_customers_paginated: remove p_allowed_sales_rep_ids filter (all users see all customers)
