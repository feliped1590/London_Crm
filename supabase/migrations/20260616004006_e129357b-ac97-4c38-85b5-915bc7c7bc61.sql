...
BEGIN
  ...
  -- Resolve UUID -> "label" das tabelas de lookup (rótulo legível esperado pelo ERP)
  IF v_result IS NOT NULL AND p_path IN ('impressao.cilindro_id','impressao.maquina_id') THEN
    ...
    IF v_uuid IS NOT NULL THEN
      IF p_path = 'impressao.cilindro_id' THEN
        SELECT label INTO v_lookup FROM public.product_ficha_cylinders WHERE id = v_uuid;
      ELSE
        SELECT label INTO v_lookup FROM public.product_ficha_machines WHERE id = v_uuid;
      END IF;
...
-- Reprocessa os valores já armazenados (que hoje têm o código numérico) para o rótulo legível
DO $$
DECLARE
  r RECORD;
