CREATE OR REPLACE FUNCTION public.save_pipeline_with_entities(_pipeline_id uuid, _name text, _description text, _type text, _is_active boolean, _allowed_roles text[], _pipeline_mode text, _legal_entity_ids uuid[])
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF _legal_entity_ids IS NOT NULL AND array_length(_legal_entity_ids, 1) > 0 THEN
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.legal_entities le WHERE le.id = eid AND le.active = true
    );
