...
BEGIN
    ...
    BEGIN
      ...
      ELSE
        INSERT INTO products (
          ...
          rec.data_alteracao::text, now(), 'ERP', true
        );
      END IF;
  ...
  -- Update sync control using correct column names
  INSERT INTO erp_sync_control (entity, last_sync_at, last_sync_count)
  VALUES ('product_promotion', now(), v_promoted)
  ON CONFLICT (entity)
  DO UPDATE SET
    last_sync_at = now(),
    last_sync_count = v_promoted;
